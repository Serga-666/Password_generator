...
# punctuation = '!#$%&*+-=?@^_'
# bad_symbols = 'il1Lo0O'  # Символы, которые могут быть путаны
#
# # Функция загрузки или генерации ключа для шифрования
# def load_key():
...
#         print("Файл с паролями ещё не создан.")
#         return
#     with open(filename, "r", encoding="utf-8") as f:
#         data = json.load(f)
#     if not data:
...
#         data = json.load(f)
#     if not data:
#         print("Нет сохранённых паролей для удаления.")
#         return
#
#     print("\nСписок сохранённых сервисов:")
#     for i, service in enumerate(data, 1):
...
#     while True:
#         try:
#             choice = int(input("Введите номер сервиса для удаления пароля: "))
#             if 1 <= choice <= len(data):
#                 service = list(data.keys())[choice - 1]
#                 confirm = input(f"Вы уверены, что хотите удалить пароль для '{service}'? (д/н): ").strip().lower()
#                 if confirm == 'д':
#                     del data[service]
#                     with open(filename, "w", encoding="utf-8") as f:
#                         json.dump(data, f, indent=4, ensure_ascii=False)
#                     print(f"Пароль для '{service}' успешно удалён.")
#                 else:
...
#         data = json.load(f)
#     if not data:
#         print("Нет сохранённых паролей для изменения.")
#         return
#
#     print("\nСписок сохранённых сервисов:")
#     for i, service in enumerate(data, 1):
...
#                 print("2. Сгенерировать новый")
#                 method = input("Ваш выбор: ").strip()
#
#                 if method == "1":
#                     while True:
#                         new_password = input(f"Введите новый пароль для '{service}': ").strip()
#                         if new_password:
#                             break
#                         print("Пароль не может быть пустым. Попробуйте снова.")
#                 elif method == "2":
#                     chars, _, length = get_user_choices_for_one_password()
...
#                     return
#
#                 encrypted = encrypt_password(new_password, fernet)
#                 data[service] = encrypted
#                 with open(filename, "w", encoding="utf-8") as f:
#                     json.dump(data, f, indent=4, ensure_ascii=False)
#                 print(f"Пароль для '{service}' успешно изменён.")
#                 break
#             else:
#                 raise ValueError()
#         except ValueError:
#             print("Некорректный ввод. Попробуйте снова.")
#
# # Запрос параметров генерации для одного пароля
# def get_user_choices_for_one_password():
#     chars = ''
#     while True:
#         try:
...
#
#     while True:
#         add_digit = input('Включить цифры? (д/н): ').strip().lower()
#         if add_digit in ('д', 'н',  'l', 'y'):
#             break
#         print("Введите 'д' или 'н'.")
#
#     while True:
#         add_lowercase = input('Включить строчные буквы? (д/н): ').strip().lower()
#         if add_lowercase in ('д', 'н',  'l', 'y'):
#             break
#         print("Введите 'д' или 'н'.")
#
#     while True:
#         add_uppercase = input('Включить заглавные буквы? (д/н): ').strip().lower()
#         if add_uppercase in ('д', 'н',  'l', 'y'):
#             break
#         print("Введите 'д' или 'н'.")
#
#     while True:
#         add_punctuation = input('Включить спецсимволы (!#$%& и т.п.)? (д/н): ').strip().lower()
#         if add_punctuation in ('д', 'н',  'l', 'y'):
#             break
#         print("Введите 'д' или 'н'.")
#
#     while True:
#         add_bad_symbols = input('Оставить символы "il1Lo0O"? (д/н): ').strip().lower()
#         if add_bad_symbols in ('д', 'н',  'l', 'y'):
#             break
#         print("Введите 'д' или 'н'.")
#
#     if add_digit == 'д':
...
#     while True:
#         print("\nВыберите действие:")
#         print("1. Сгенерировать и сохранить пароли")
#         print("2. Добавить существующий пароль вручную")
#         print("3. Посмотреть сохранённые пароли")
#         print("4. Удалить сохранённый пароль")
#         print("5. Изменить сохранённый пароль")
#         print("6. Выйти")
#         choice = input("Ваш выбор: ").strip()
#
#         if choice == "1":
#             chars, n, length = get_user_choices()
#             for i in range(n):
#                 password = generate_password(length, chars)
#                 print(f"\nПароль {i + 1}: {password}")
#                 service = input("Для какого сервиса сохранить этот пароль? ").strip()
#                 encrypted = encrypt_password(password, fernet)
#                 save_password(service, encrypted)
#                 print(f"Пароль сохранён для сервиса '{service}'.")
#
#         elif choice == "2":
#             service = input("Введите название сервиса: ").strip()
#             password = input("Введите уже существующий пароль: ").strip()
#             encrypted = encrypt_password(password, fernet)
#             save_password(service, encrypted)
#             print(f"Сохранён вручную введённый пароль для '{service}'.")
#
#         elif choice == "3":
...
#             print("Некорректный выбор. Попробуйте снова.")
#
# # Функция генерации нескольких паролей (для опции 1)
...
# bad_symbols = 'il1Lo0O'  # Символы, которые могут быть путаны
#
# # Загрузка или генерация ключа для шифрования
# def load_key():
#     if not os.path.exists("key.key"):
#         key = Fernet.generate_key()
#         with open("key.key", "wb") as key_file:
...
#             print(f"Ошибка ввода: {e}")
#
#     # Включение/исключение групп символов
#     def ask_choice(prompt, yes_chars, no_chars):
#         while True:
#             choice = input(prompt).strip().lower()
#             if choice in yes_chars:
#                 return True
#             elif choice in no_chars:
#                 return False
#             else:
#                 print("Введите 'д' или 'н'.")
#
#     if ask_choice('Включить цифры? (д/н): ', ('д','l'), ('н','y')):
#         chars += digits
#     if ask_choice('Включить строчные буквы? (д/н): ', ('д','l'), ('н','y')):
#         chars += lowercase_letters
#     if ask_choice('Включить заглавные буквы? (д/н): ', ('д','l'), ('н','y')):
#         chars += uppercase_letters
#     if ask_choice('Включить спецсимволы (!#$%& и т.п.)? (д/н): ', ('д','l'), ('н','y')):
#         chars += punctuation
#     if not ask_choice('Оставить символы "il1Lo0O"? (д/н): ', ('д','l'), ('н','y')):
#         for ch in bad_symbols:
#             chars = chars.replace(ch,'')
#
#     return chars, n, length
#
# # Универсальная генерация пароля
# def generate_password(length, chars):
#     return ''.join(random.choice(chars) for _ in range(length))
#
# # Изменение пароля
# def change_password(fernet):
#     filename = "passwords.json"
#     if not os.path.exists(filename):
#         print("Файл с паролями ещё не создан.")
#         return
...
#
#     # Копируем файлы
#     files_to_backup = ["passwords.json", "key.key"]
...
#
#     try:
#         choice = int(input("Выберите номер резервной копии для восстановления: "))
#         if 1 <= choice <= len(backups):
#             backup_file = backups[choice - 1]
#             timestamp = backup_file.split('_')[0]
#
#             # Восстанавливаем файлы
#             shutil.copy2(os.path.join(backup_dir, backup_file), "passwords.json")
#             key_file = f"{timestamp}_key.key"
#             if os.path.exists(os.path.join(backup_dir, key_file)):
#                 shutil.copy2(os.path.join(backup_dir, key_file), "key.key")
#
...
#         data = json.load(f)
#
#     # Расшифровываем все пароли для экспорта
#     decrypted_data = {}
#     for service, encrypted_password in data.items():
#         decrypted_data[service] = decrypt_password(encrypted_password, fernet)
#
#     export_filename = input(
#         "Введите имя файла для экспорта (по умолчанию: passwords_export.json): ") or "passwords_export.json"
#
...
#
# # Импорт данных
# def import_data(fernet):
#     """Импорт данных из файла"""
#     import_filename = input(
#         "Введите имя файла для импорта (по умолчанию: passwords_import.json): ") or "passwords_import.json"
#
...
#         existing_data = {}
#
#     # Шифруем и добавляем импортированные данные
#     imported_count = 0
#     for service, password in imported_data.items():
#         if service not in existing_data:
#             encrypted = encrypt_password(password, fernet)
#             existing_data[service] = encrypted
#             imported_count += 1
#
...
#         print("4. Удалить сохранённый пароль")
#         print("5. Изменить сохранённый пароль")
#         print("6. Резервное копирование данных")
#         print("7. Восстановление из резервной копии")
#         print("8. Экспорт данных")
#         print("9. Импорт данных")
#         print("0. Выйти")
#         print("=" * 50)
#
#         choice = input("Ваш выбор: ").strip()
